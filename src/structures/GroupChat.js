...
class GroupChat extends Chat {
   ...
   * @param {Array<string>} participantIds
   * @return {Promise<{ status: number }>} Object with status code indicating if
  ...
  async setSubject(subject) {
    const res = await this.client.pupPage.evaluate((chatId, subject) => {
  ...
  async setDescription(description) {
    const res = await this.client.pupPage.evaluate((chatId, description) => {
      const descId = window.Store.GroupMetadata.get(chatId).descId;
      return window.Store.Wap.setGroupDescription(chatId, description,
          window.Store.genId(), descId);
   ...
   * @return {Promise<boolean>} Returns true if the setting was properly
   * updated. This can return false if the user does not have the necessary
   * permissions.
   */
  async setMessagesAdminsOnly(adminsOnly = true) {
    const res = await this.client.pupPage.evaluate((chatId, value) => {
  ...
  /**
   * Updates the group settings to only allow admins to edit group info (title,
   * description, photo).
   * @param {boolean} [adminsOnly=true] Enable or disable this option
   * @return {Promise<boolean>} Returns true if the setting was properly
   * updated. This can return false if the user does not have the necessary
   ...
   * @return {Promise<string>} Group's invite code
   */
  async getInviteCode() {
    const res = await this.client.pupPage.evaluate((chatId) => {
